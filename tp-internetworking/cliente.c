...
int main (int argc, char *argv[]) {
    ...
    while (flag_nombre) {
        ...
        if (fgets(nickname, LENGTH_NAME, stdin) != NULL) {  //almacena la cadena introducida por teclado
    ...
    if (pthread_create(&send_msg_thread, NULL, (void *) send_mensaje, cliente) != 0) { // para llamar a la funcion send_mensaje el parametro no se puede 
    ...
    if (pthread_create(&recv_msg_thread, NULL, (void *) recv_mensaje, cliente) != 0) { //lo mismo aca cuando se llama a recv_mensaje
        printf ("Error en pthread!\n");
        exit(-1);
    }
